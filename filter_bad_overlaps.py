...
X_check = X
y_check = y
eprint("Examples taken to build: {}".format(y_train.count(1)))

eprint("starting classification")
#clf = SVC(kernel='rbf')
...
eprint(sklearn.metrics.precision_score(y_check, y_pred))
eprint(sklearn.metrics.recall_score(y_check, y_pred))
eprint(sklearn.metrics.accuracy_score(y_check, y_pred))
removed_overlaps = 0
...
eprint(removed_invalid_overlaps, removed_overlaps, kept_invalid_overlaps)
eprint(skipped, invalid_skipped)

for i in range(len(y_pred)):
    if y_pred[i] == 1:
        print(y_overlap_index_mapping[i])
